...
//***************************************************************************
int LED_BUILTIN=2;
const char* WIFI_SSID = "ZTE-e25ff5";
const char* WIFI_PASS = "fliabalderrama19";
...
void callback(const char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == SUBCRIBE_TOPIC) {
    ...
    if (!err) {
      ...
      if (action == "Alarm_on") sonido();
      else if (action == "Alarm_off") noTone();
...
StaticJsonDocument<JSON_OBJECT_SIZE(5)> outputDoc;
